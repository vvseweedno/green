...
def _asset_scale_offset(item, key: str) -> tuple[float, float]:
    bands = item.assets[key].extra_fields.get("raster:bands") or []
    if bands:
...
def read_asset_on_20m_grid(item, geometry_wgs84, asset_key: str, *, reference_key: str = "nir08"):
    ...
    with rasterio.open(href) as src:
        with WarpedVRT(
            src, crs=ref_crs, transform=ref_transform, width=width, height=height,
...
def _inside_aoi_mask(geometry_wgs84, transform, crs, shape):
    geom = _geometry_in_crs(geometry_wgs84, crs)
    return geometry_mask([mapping(geom)], out_shape=shape, transform=transform, invert=True)


def assess_candidate(item, geometry_wgs84) -> Candidate:
    scl, transform, crs = read_asset_on_20m_grid(item, geometry_wgs84, "scl")
    inside = _inside_aoi_mask(geometry_wgs84, transform, crs, scl.shape)
    q = scl_quality_summary(scl[inside])
    cc = item.properties.get("eo:cloud_cover")
    return Candidate(
        item, q["valid_fraction"], q["strict_valid_fraction"],
        q["low_confidence_fraction"], cc
    )


def rank_candidates(items, geometry_wgs84, *, max_assessed: int = 12) -> list[Candidate]:
    # Tile cloud cover is only a pre-filter. Final ordering is by SCL strictly inside the AOI.
    pre = sorted(
        items, key=lambda it: float(it.properties.get("eo:cloud_cover", 100.0))
    )[:max_assessed]
    assessed = [assess_candidate(it, geometry_wgs84) for it in pre]
    return sorted(
        assessed,
        key=lambda c: (
            c.strict_valid_fraction,
            c.valid_fraction,
            -(c.tile_cloud_cover or 100.0),
        ),
        reverse=True,
    )
...
def read_scene(item, geometry_wgs84) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    ...
    for public_name, asset_key in ASSETS.items():
        ...
        if asset_key != "scl":
            scales[public_name] = dict(
                zip(("scale", "offset"), _asset_scale_offset(item, asset_key))
            )
    if transform is not None and crs is not None:
        inside = _inside_aoi_mask(
            geometry_wgs84, transform, crs, arrays["SCL"].shape
        )
        arrays["SCL"] = np.where(inside, arrays["SCL"], 0).astype(np.uint8)
        for name in ("B02", "B03", "B04", "B8A", "B11", "B12"):
            arrays[name] = np.where(inside, arrays[name], np.nan).astype(np.float32)
    metadata = {
        "item_id": item.id,
        "datetime": (
