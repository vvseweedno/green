...
def connected_objects(
    ...
    *,
    ...
    for label_id in range(1, n + 1):
        ...
        geoms = [
            shape(g)
            for g, value in shapes(obj.astype(np.uint8), mask=obj, transform=transform)
            if value == 1
        ]
        if not geoms:
            continue
        ...
        for g in geoms[1:]:
            geom = geom.union(g)
        geom_wgs84 = reproject_geometry(geom, crs, "EPSG:4326")
        area = geodesic_area_ha(geom_wgs84)
        if area < min_area_ha:
            continue
        out.append(
            ChangeObject(
                ...
                area,
                int(obj.sum()),
                float(np.nanmean(np.asarray(score, dtype=float)[obj])),
            )
        )
    return out
